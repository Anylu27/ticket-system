...
<ul class="sidebar-menu">
    ...
    <li>
        <a href="#" onclick="toggleSubmenu('maestros'); return false;" class="<?php echo in_array($current_page, ['maestros', 'clientes', 'proyectos', 'categorias', 'ubicaciones', 'areas']) ? 'active' : ''; ?>">
    ...
    <li>
        <a href="#" onclick="toggleSubmenu('seguridad'); return false;" class="<?php echo in_array($current_page, ['usuarios', 'bitacora']) ? 'active' : ''; ?>">
...
function inicializarSidebar() {
    ...
    if (maestrosPages.includes(currentPage)) {
        const maestrosSubmenu = document.getElementById('maestros-submenu');
        const maestrosArrow = document.getElementById('maestros-arrow');
        if (maestrosSubmenu && maestrosArrow) {
            maestrosSubmenu.style.display = 'block';
            maestrosArrow.style.transform = 'rotate(90deg)';
        }
    }

    if (seguridadPages.includes(currentPage)) {
        const seguridadSubmenu = document.getElementById('seguridad-submenu');
        const seguridadArrow = document.getElementById('seguridad-arrow');
        if (seguridadSubmenu && seguridadArrow) {
            seguridadSubmenu.style.display = 'block';
            seguridadArrow.style.transform = 'rotate(90deg)';
        }
    }
}
</script>
